...
#
#     model = Network.build(hid_neurons=[4])
#     pinn = Pinn(model, c=1.0).pinn
#     solver = L_BFGS_B(model=pinn, x_train=inputs, y_train=outputs, maxiter=800, factr=2.5e3)
#
#     # initial_weights = np.concatenate(
#     #     [w.flatten() for w in solver.model.get_weights()])
#     #
#     # loss, grads = solver.tf_evaluate(solver.x_train, solver.y_train)
#     # # flat_grads = np.concatenate([r.numpy().flatten() for r in grads]).astype('float64')
#     # print([aaa for aaa in grads])
#     var = solver.model.trainable_variables[-1]
#
#     with tf.GradientTape() as tape:
#         tape.watch(var)
#         psi = solver.model(solver.x_train)
#         loss = tf.reduce_mean(tf.keras.losses.mse(psi, solver.y_train))
#
